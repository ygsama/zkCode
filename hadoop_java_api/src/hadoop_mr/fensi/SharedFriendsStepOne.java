...
public class SharedFriendsStepOne {
	...
	static class SharedFriendsStepOneReducer extends Reducer<Text, Text, Text, Text> {
		...
		protected void reduce(Text friend, Iterable<Text> persons, Context context) throws IOException, InterruptedException {
			...
			for (Text person : persons) {	//<B,A> <C,A> <D,A> <F,A> <O,A>
				sb.append(person).append(",");

			}
			context.write(friend, new Text(sb.toString())); //B,<A,C> B是AC的好友
		}
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputFormat.setInputPaths(job, new Path("D:/temp/srcdata/friends"));
